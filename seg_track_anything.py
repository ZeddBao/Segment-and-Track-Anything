    ...
    if id_countour:
        ...
        for id in obj_ids:
            ...
            if id <= 255:
                ...
                color = [0, 0, 0]
            foreground = img * (1-alpha) + np.ones_like(img) * \
            ...
            countours = binary_dilation(
                binary_mask, iterations=1) ^ binary_mask
...
def tracking_objects_in_video(SegTracker, input_video, input_img_seq, fps, frame_num=0):
    ...
    io_args = {
        ...
        # keep same format as input video
...
def video_type_input_tracking(SegTracker, input_video, io_args, video_name, frame_num=0):
    ...
    if frame_num > 0:
        output_mask_name = sorted(
            [img_name for img_name in os.listdir(io_args['output_mask_dir'])])
        output_masked_frame_name = sorted(
            [img_name for img_name in os.listdir(io_args['output_masked_frame_dir'])])

        for i in range(0, frame_num):
            cap.read()
            pred_list.append(np.array(Image.open(os.path.join(
                io_args['output_mask_dir'], output_mask_name[i])).convert('P')))
            masked_pred_list.append(cv2.imread(os.path.join(
                io_args['output_masked_frame_dir'], output_masked_frame_name[i])))
    ...
    ########################################
            ...
            save_prediction(pred_mask, output_mask_dir, str(
                frame_idx + frame_num).zfill(5) + '.png')
            pred_list.append(pred_mask)

            print("processed frame {}, obj_num {}".format(
                frame_idx + frame_num, SegTracker.get_obj_num()), end='\r')
            frame_idx += 1
        cap.release()
        print('\nfinished')
    ...
    out = cv2.VideoWriter(io_args['output_video'],
    ...
    while cap.isOpened():
        ...
        cv2.imwrite(
            f"{io_args['output_masked_frame_dir']}/{str(frame_idx).zfill(5)}.png", masked_frame[:, :, ::-1])
    ...
    # zip predicted mask
    os.system(
        f"zip -r {io_args['tracking_result_dir']}/{video_name}_pred_mask.zip {io_args['output_mask_dir']}")
...
def img_seq_type_input_tracking(SegTracker, io_args, video_name, imgs_path, fps, frame_num=0):
    ...
    with torch.cuda.amp.autocast():
        for img_path in imgs_path:
            ...
            if frame_idx == 0:
                ...
                # find new objects, and update tracker with new objects
                new_obj_mask = SegTracker.find_new_objs(track_mask, seg_mask)
                save_prediction(new_obj_mask, output_mask_dir,
                                f'{frame_name}_new.png')
                pred_mask = track_mask + new_obj_mask
                # segtracker.restart_tracker()
                SegTracker.add_reference(frame, pred_mask)
            else:
    ...
    for img_path in imgs_path:
        ...
        masked_pred_list.append(masked_frame)
        cv2.imwrite(
            f"{io_args['output_masked_frame_dir']}/{frame_name}.png", masked_frame[:, :, ::-1])
    ...
    # save colorized masks as a gif
    imageio.mimsave(io_args['output_gif'],
                    masked_pred_list, duration=1000 / fps)
    print("{} saved".format(io_args['output_gif']))
