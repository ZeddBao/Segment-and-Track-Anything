...
class SegTracker():
    ...
    def find_new_objs(self, track_mask, seg_mask):
        ...
        for idx in new_obj_ids:
            ...
            if new_obj_area/obj_area < self.min_new_obj_iou or new_obj_area < self.min_area or obj_num > self.max_obj_num:
    ...
    def detect_and_seg(self, origin_frame: np.ndarray, grounding_caption, box_threshold, text_threshold, box_size_threshold=1, reset_image=False):
        ...
        annotated_frame, _, annotated_boxes = self.detector.run_grounding(origin_frame, grounding_caption, box_threshold, text_threshold)
        annotated_masks = []
        refined_merged_mask = np.zeros(origin_frame.shape[:2], dtype=np.uint8)
        for i in range(len(annotated_boxes)):
            bbox = annotated_boxes[i][0]
            ...
            annotated_masks.append({'mask': None, 'bbox': [bbox[0][0], bbox[0][1], bbox[1][0], bbox[1][1]],
                                   'logit': annotated_boxes[i][1], 'phrase': annotated_boxes[i][2], 'id': None})
            refined_merged_mask = self.add_mask(interactive_mask)
        ...
        return refined_merged_mask, annotated_frame, annotated_masks
